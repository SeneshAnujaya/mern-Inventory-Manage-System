  ...
  return (
    <div className={sidebarClassNames}>
      ...
      <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
        <div>logo</div>
        <h1 className={`font-extrabold text-2xl ${isSidebarCollapsed ? "hidden":"block"}`}>STOKES</h1>

        <button className="md:hidden px-3 py-3 bg-gray-100 rounded-full hover:bg-blue-100" onClick={toggleSidebar}><Menu className="w-4 h-4"/></button>
      </div>
      ...
      <div className={`${isSidebarCollapsed ? "hidden":"block"} mb-10 `}>
        <p className={`text-center text-gray-500 text-xs`}>&copy; 2024 Stokes</p>
      </div>
    </div>
  );
};
